...
{    
    ...
    pBot->del_x = pBot->X_pos1 - pBot->prev_X - DegreeToRadian(delYaw)*RadiusXYCircle;  
  
     
    pBot->Y_pos1 = (pEncoderY->Count * 2 * pi * RadiusXYWheel / pEncoderY->ppr); //cm
    pBot->del_y = pBot->Y_pos1 - pBot->prev_Y + DegreeToRadian(delYaw)*RadiusXYCircle;
    
    float r = dist(0,0,pBot->del_x,pBot->del_y);
    float Theta = the(pBot->del_y,pBot->del_x,robotYaw);
