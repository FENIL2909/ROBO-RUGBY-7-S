...
enum FSM {arenaSelect, waitForRefree, receiveBall, Collaborate, placeBall, resetState, variableState, goHome, kickBall, manualDrive};
enum zone {startZone = -1 ,receiveZone = 1};
FSM fsmState = arenaSelect;

int speedMode = constant;
int initYaw = 0, timerMPU = 0;

bool collabFlag = 0, botStop = 0, startPlanning = 0, driftFlag = 0;
...
float kickingX = 845.5, kickingY = 256.0;
float receiveX = 930.5, receiveY = 60; //845.5,38
...
//  ETAgentTR.sendData();
  ...
  // p d i req min max
  ppidOmega->initPID(0.13,1.0,0, 0, -2, 2);   //0.12, 0.1       
  ppidDistance->initPID(7.50,0,0,0,-400,400);
  ppidCircle->initPID(15,5,0,0,-400,400);
